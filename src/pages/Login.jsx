...
const Login = () => {
  ...
  // Watch email value for resend confirmation
  ...
  useEffect(() => {
    if (showResendConfirmation && (watchedEmail || watch('password'))) {
      setShowResendConfirmation(false)
      setResendMessage('')
    }
    
    // Clear confirmation message when user starts typing
    if (showConfirmationMessage && (watchedEmail || watch('password'))) {
      setShowConfirmationMessage(false)
    }
  }, [watchedEmail, watch('password'), showResendConfirmation, showConfirmationMessage])
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (result?.type === 'auth/login/fulfilled' || result?.success === true) {
        navigate('/dashboard')
        return
      }
      
      // Handle login errors
      if (result?.type === 'auth/login/rejected' || result?.success === false) {
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium">
                    Confirmation link sent to your inbox. Please check your email and click the link to verify your account.
                  </p>
                </div>
              </div>
            </div>
